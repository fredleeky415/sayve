...
export type SupabaseMigrationInventoryRow = {
  line: number;
  version: string;
  file: string;
  requiredFor: "private_beta" | "public_launch";
...
export function getSupabaseMigrationInventory(): SupabaseMigrationInventoryRow[] {
  return readdirSync(migrationDir)
    .filter((file) => file.endsWith(".sql"))
    .sort((a, b) => a.localeCompare(b))
    .map((file, index) => {
      const metadata = migrationNotes[file] ?? {
        requiredFor: "public_launch" as const,
